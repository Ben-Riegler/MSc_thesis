...
__all__ = ['GPGenerator', 'SawtoothGenerator', "SinGenerator", "StepGenerator"]
...
class DataGenerator(metaclass=abc.ABCMeta):
    ...
    def generate_task(self, n_ctxt = None):
        """Generate a task.
...
class SinGenerator(DataGenerator):
    """Generate samples from a random sin-wave.
    ...
    def sample(self, x):
        # Sample parameters of sin.
         
        amp = _rand(self.amp_dist) 
        per = _rand(self.per_dist) 
        shift = _rand(self.shift_dist)
        lift = _rand(self.lift_dist)
...
class StepGenerator(DataGenerator):
    """Generate samples from a random step-fct.
    ...
    def __init__(self,
        ...
        self.jump_ran = jump_ran

    def sample(self, x):
        # Sample parameters of sin.
         
        c = _rand(self.jump_ran)

        # Construct expansion.

        x = x[:, None] #(N,1)

        fs = np.where(x[:, 0] < c, np.zeros_like(x[:, 0]), np.ones_like(x[:, 0]))

        return np.reshape(fs, (-1,1))
